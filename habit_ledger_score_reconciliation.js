;(function installHabitLedgerScoreReconciliation(global) {
  ...
  function simulateFullRepair(state, fullPlan, dependencies = {}) {
    ...
    try {
      // The score-reconciliation layer independently validates every projected
      // matchup result, then updates the redundant score copies in the same state.
      // Use the original deterministic ledger apply only for this in-memory preview.
