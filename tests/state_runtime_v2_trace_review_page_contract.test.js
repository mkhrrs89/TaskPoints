...
test('foreground correlator ties legacy full-state work to the interaction-to-enqueue window', () => {
  ...
  assert.equal(result.legacyForegroundCandidateCount, 2);
  assert.equal(result.maxLegacyForegroundCandidateMs, 35);
  assert.equal(result.byMutationClass.completion.fullyCorrelated, true);
  assert.deepEqual(result.mutationWindows[0].candidates.map((row) => row.name), ['phase2.persist', 'storage.setItem']);
});
