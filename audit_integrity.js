(function (global) {
  ...
  function buildNpcScoreHealthAudit(state, options = {}) {
    const out = issueCollector(), min = finiteValue(options.npcScoreMin) ? Number(options.npcScoreMin) : 5, max = finiteValue(options.npcScoreMax) ? Number(options.npcScoreMax) : 86, today = validLedgerKey(options.todayKey) ? options.todayKey : '';
  ...
  function buildMatchupHistoryReconciliationAudit(state, options = {}) {
    ...
    expectations.forEach(expected => {
      ...
      if (idMatches.length > 1) { out.fail(`${expected.label} has ${idMatches.length} gameHistory rows with explicit matchup ID ${shortId(expected.matchupId)}`, 2); idMatches.forEach(found => unused.delete(found)); return; }
  ...
  function buildHabitLedgerConsistencyAudit(state, options = {}) {
    const out = issueCollector(), habits = Array.isArray(state && state.habits) ? state.habits : [], byId = new Map(), ledgers = new Map();
    ...
    const youMatchups = (Array.isArray(state && state.matchups) ? state.matchups : [])
      .map((matchup, index) => {
        ...
        return date && score.valid ? { matchup, index, date, score: score.value } : null;
      })
      .filter(Boolean);
    const earliestYouMatchupDay = youMatchups.length
      ? youMatchups.map(item => item.date).sort()[0]
      : '';

    missingByDay.forEach((rows, day) => {
      const sample = rows.slice(0, 5).map(item => item.label).join(', ');
      if (day === options.todayKey) {
        out.warn(`${rows.length} current doneKey(s) on ${day} have no completion row. Sample: ${sample}.`, 2);
        return;
      }

      const dayGames = youMatchups.filter(item => item.date === day);
      const currentTotal = dailyTotals && finiteValue(dailyTotals[day]) ? Number(dailyTotals[day]) : NaN;
      if (dayGames.length === 1 && Number.isFinite(currentTotal) && Math.abs(dayGames[0].score - currentTotal) <= 0.05) {
        return;
      }

      if (!dayGames.length && earliestYouMatchupDay && day < earliestYouMatchupDay) {
        return;
      }

      if (dayGames.length) {
        const stored = dayGames.length === 1 ? formatScore(dayGames[0].score) : `${dayGames.length} stored You matchups`;
        const current = Number.isFinite(currentTotal) ? formatScore(currentTotal) : 'unavailable';
        out.warn(`${rows.length} historical doneKey(s) on ${day} have no completion row and cannot be proven score-neutral: stored game ${stored}, current ledger ${current}. Sample: ${sample}.`, 4);
        return;
      }

      out.warn(`${rows.length} historical doneKey(s) on ${day} have no completion row and no stored You matchup is available to verify score impact. Sample: ${sample}.`, 4);
    });

    const result = out.result(options); return { id: 'habit-ledger-consistency', title: 'Habit ledger consistency', section: 'Habit Ledger Integrity', status: result.status, expected: 'Habit/Vice done, failed, ice, and completion records agree by habit and date', actual: result.summary, details: result.details, trace: 'state.habits doneKeys/failedKeys/iceKeys ↔ state.completions source/habitId/dayKey', tips: 'Historical doneKeys without completion rows are ignored only when a stored You game score still matches the canonical completion ledger, or when the marker predates the first stored You matchup. Current-day, post-game-era unverified, and score-discrepant missing rows still warn. Historical stored points are preserved because Habit/Vice values may change over time. This audit does not create or delete completion entries.' };
  }
